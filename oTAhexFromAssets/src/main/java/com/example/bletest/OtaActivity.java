...
public class OtaActivity extends Activity implements OnClickListener ,LoaderCallbacks<Cursor>{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		if(!TextUtils.isEmpty(Tools.device.getName())&&Tools.device.getName().toLowerCase().contains("dfu")){
	...
	private void initView() {
		...
		findViewById(R.id.bt_ota).setOnClickListener(this);
	...
	private void startUpdate() {
		if(!TextUtils.isEmpty(Tools.device.getName())&&Tools.device.getName().toLowerCase().contains("dfu")){
			sendHex(Tools.device.getName(),Tools.device.getAddress());
		}else{			
			if (device == null)
				return;
...
//					mTextUploading.setText(R.string.dfu_status_uploading);
	...
	private void showErrorMessage(int code) {
		// TODO Auto-generated method stub
		showToast("Upload failed: " + GattError.parse(code) + " (" + (code & ~(DddService.ERROR_MASK | DddService.ERROR_REMOTE_MASK)) + ")");
	}
	...
	private void sendHex(String name,String address) {
		...
		String type="";
		if(Tools.OTAtype==1){
			mFilePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + DeviceScanActivity.FilePath;
			file=mFilePath;
		}else{
			 file=mTvFile.getText().toString();
		}
	...
	//	mFilePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + "v0_6_4_b018_jstyle_limited.hex";
		Intent intent = new Intent(this, DddService.class);
...
//		}
	//	findHexPath();
		...
		intent.putExtra(DfuBaseService.EXTRA_FILE_TYPE, mFileType);
		intent.putExtra(DfuBaseService.EXTRA_FILE_URI, mFileStreamUri);
		startService(intent);
	...
	private void showRestDialog(){
		...
		.setTitle("Reset")
		.setMessage("Update Complete,do you need hibernate the device?")
	...
	protected void resetFactoryMode() {
		// TODO Auto-generated method stub
		isReset=true;
		device.FactoryReset();
	}
